...
def convert_box_to_mask(outputs_box: torch.Tensor, h: int, w: int):
    box_normalizer = torch.as_tensor([w, h, w, h]).to(outputs_box).reshape(1, 1, -1)
...
class MultiScaleMaskedTransformerDecoder(nn.Module):
    ...
    def from_config(cls, cfg, in_channels, mask_classification):
        ...
        ret["boxvis_enabled"] = cfg.MODEL.BoxVIS.BoxVIS_ENABLED
        ret["boxvis_boxdet_on"] = cfg.MODEL.BoxVIS.BOXDET_ON

        return ret

    def forward(self, x, mask_features, clip_mask_features=None, mask=None):
        ...
        outputs_class, outputs_mask, attn_mask = \
            self.forward_prediction_heads(output, mask_features, attn_mask_target_size=size_list[0])
        ...
        for i in range(self.num_layers):
            ...
            outputs_class, outputs_mask, attn_mask = \
                self.forward_prediction_heads(output, mask_features, attn_mask_target_size)
            predictions_class.append(outputs_class)
            predictions_mask.append(outputs_mask)
        ...
        return out, self.decoder_norm(output).permute(1, 0, 2), mask_features
    ...
    def forward_prediction_heads(self, output, mask_features, attn_mask_target_size):
        ...
        attn_mask = repeat(attn_mask, 'B Q H W -> (B h) Q (H W)', h=self.num_heads).bool()
    ...
    @torch.jit.unused
    def _set_aux_loss_with_box(self, outputs_class, outputs_box, outputs_seg_masks):
        # this is a workaround to make torchscript happy, as torchscript
        # doesn't support dictionary with non-homogeneous values, such
        # as a dict having both a Tensor and a list.
        if self.mask_classification:
            return [
                ...
                for c, b, m in zip(outputs_class[:-1], outputs_box[:-1], outputs_seg_masks[:-1])
            ]
        else:
            return [{"pred_boxes": b, "pred_masks": m} for b, m in zip(outputs_box[:-1], outputs_seg_masks[:-1])]
