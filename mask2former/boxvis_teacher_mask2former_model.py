...
from mask2former.modeling.criterion import SetCriterion, TeacherSetPseudoMask
from mask2former.modeling.matcher import HungarianMatcher
from mask2former.utils.box_ops import box_xyxy_to_cxcywh


@META_ARCH_REGISTRY.register()
class BoxVIS_Teacher_MaskFormer(nn.Module):
    """
    ...
    def from_config(cls, cfg):
        ...
        gen_pseudo_mask = TeacherSetPseudoMask(matcher)
        ...
        if update_teacher:
            ...
            for name, param in self.backbone_t.named_parameters():
                if name in backbone_shadow:
                    param.data = (1.0 - self.ema_shadow_decay) * backbone_shadow[name] \
                                 + self.ema_shadow_decay * param.data
            for name, param in self.sem_seg_head_t.named_parameters():
                if name in sem_seg_head_shadow:
                    param.data = (1.0 - self.ema_shadow_decay) * sem_seg_head_shadow[name] \
                                 + self.ema_shadow_decay * param.data

        features_t = self.backbone_t(images_norm.tensor)
        outputs_t, frame_queries_t, mask_features_t = self.sem_seg_head_t(features_t)

        # generate pseudo masks
        frame_targets = self.gen_pseudo_mask(outputs_t, frame_targets)

        display_mask = False
        if display_mask and frame_targets[0]["masks_pseudo"].shape[0] > 0:
            img_name = batched_inputs[0]['file_names'][0].split('/')[-1][:-4]
            scores, idx = frame_targets[0]["mask_pseudo_scores"].sort(descending=True)
            masks_pseudo = frame_targets[0]["masks_pseudo"]
            root_dir = 'output/coco/boxvis_teacher_mask2former_r50_coco_f1/pseudo/'
            if not os.path.exists(root_dir):
                os.makedirs(root_dir)
            im = images.tensor[0].to(masks_pseudo.device)
            plt.imshow(im.byte().permute(1, 2, 0).cpu().numpy())
            plt.savefig(root_dir + img_name + '_img.jpg')
            n = min(masks_pseudo.shape[0], 4)
            im_ = rearrange(im[None].repeat(n, 1, 1, 1), '(K R) C H W -> (K H) (R W) C', R=n)
            mask_ = rearrange(masks_pseudo[idx[:n]], '(K R) H W -> (K H) (R W) 1', R=n)
            plt.imshow((im_ * mask_).byte().cpu().numpy())
            plt.savefig(root_dir + img_name + '_mask.jpg')

        # ------------------ Student Net -----------------------------
        features_s = self.backbone(images_norm.tensor)
        outputs_s, frame_queries, mask_features_s = self.sem_seg_head(features_s)

        # bipartite matching-based loss
        losses, fg_indices = self.criterion(outputs_s, frame_targets)
    ...
    def inference(self, batched_inputs):
        ...
        features_t = self.backbone_t(images.tensor)
        outputs, frame_queries_t, mask_features = self.sem_seg_head_t(features_t)

        # features = self.backbone(images.tensor)
        # outputs, frame_queries_s, mask_features = self.sem_seg_head(features)
        ...
        if self.boxvis_enabled and self.boxvis_boxdet_on:
            mask_box = outputs["pred_boxes"][0]   # cQ, 4
        ...
        if display_mask:
            ...
            root_dir = 'output/coco/boxvis_teacher_mask2former_r50_coco_f1/9_teacher_pair/'
            if not os.path.exists(root_dir):
                os.makedirs(root_dir)
            im = batched_inputs["image"][0].to(mask_pred.device)
            plt.imshow(im.byte().permute(1, 2, 0).cpu().numpy())
            plt.savefig(root_dir + img_name + '_img.jpg')
            im_ = rearrange(im[None].repeat(6, 1, 1, 1), '(K R) C H W -> (K H) (R W) C', K=2)
            mask_ = rearrange(mask_pred[idx[:6], 0].sigmoid(), '(K R) H W -> (K H) (R W) 1', K=2)
            plt.imshow((im_ * mask_.gt(0.5)).cpu().numpy())
            plt.savefig(root_dir + img_name + '_mask.jpg')

        mask_pred = F.interpolate(
            mask_pred, size=(out_height, out_width), mode="bilinear", align_corners=False
        )
        # mask_pred = mask_pred.gt(0.)
        mask_pred = mask_pred.sigmoid().gt(0.5)
        ...
        if self.boxvis_enabled and self.boxvis_boxdet_on:
            mask_box = mask_box[topk_indices]
            rate_h, rate_w = interim_size[0] / image_size[0], interim_size[1] / image_size[1]
            mask_box = mask_box * torch.as_tensor([rate_w * out_width, rate_h * out_height,
                                                   rate_w * out_width, rate_h * out_height]
                                                  ).reshape(1, -1).to(mask_box)
            result.pred_boxes = Boxes(mask_box.long())
        else:
    ...
    def display_masks(self, batched_inputs, scores_per_video, masks_pred):
        ...
        root_dir = 'output/coco/minvis_boxvisft+ema_r50_ytvis21_f1_nolsj/9/' + video_name + '/'
        if not os.path.exists(root_dir):
            os.makedirs(root_dir)
        scores, idx = scores_per_video.sort(descending=True)
        for i in range(5):
            ...
            # plt.savefig(root_dir + str(j) + '_img.jpg')
            plt.imshow(rearrange(masks_pred[idx[i], :6*(masks_pred.shape[1]//6)].sigmoid().gt(0.5),
                                 '(K R) H W -> (K H) (R W)', R=6).cpu().numpy())
