...
__all__ = ["_get_refcoco_meta", "load_refcoco_json", "convert_to_coco_json", "register_refcoco"]

# REFCOCO_CATEGORIES = [{"color": [220, 20, 60], "isthing": 1, "id": 1, "name": "object"}] # only one class for visual grounding
...
def load_refcoco_json(json_file, image_root, dataset_name=None, extra_annotation_keys=None):
    ...
    #   'category_id': 16,
    #   'id': 42986,  # identify object ids, an object may have multiple expressions
    #   'exp_id': 2   # a unique exp id for each expression
    #   }]
    ...
    if "minival" not in json_file:
        ...
        ann_ids = [ann["exp_id"] for anns_per_image in anns for ann in anns_per_image]
        assert len(set(ann_ids)) == len(ann_ids), "Annotation ids in '{}' are not unique: {} and {}".format(
            json_file, len(set(ann_ids)), len(ann_ids)
        )
    ...
    for (img_dict, anno_dict_list) in imgs_anns:
        ...
        for anno in anno_dict_list:
            ...
            objs.append(obj)
            exp_ids.append(anno["exp_id"])
            expressions.append(anno["expression"])
        
        record["annotations"] = objs
        record["exp_id"] = exp_ids
        record["expressions"] = expressions
        record["dataset_name"] = 'refcoco'
...
if __name__ == "__main__":
    ...
    dicts = load_refcoco_json(sys.argv[1], sys.argv[2], sys.argv[3])
