...
def annotations_to_instances(annos, image_size, mask_format="polygon"):
    ...
    if len(annos) and "bbox" in annos[0]:
        boxes = (
            np.stack([
                BoxMode.convert(obj["bbox"], obj["bbox_mode"], BoxMode.XYXY_ABS)
                if obj["bbox"] is not None else np.zeros(4) for obj in annos
            ])
        )
        target.gt_boxes = Boxes(boxes)

    if len(annos) and "category_id" in annos[0]:
        # SA-1B does not include object categories
