...
class FixedSizeCropClip(T.Augmentation):
    ...
    def __init__(self, crop_size: Tuple[int], pad: bool = True, pad_value: float = 0.0, clip_frame_cnt=1, pseudo_temporal_shift=True):
        """
        Args:
    ...
    def _get_crop(self, image: np.ndarray):
        ...
        if self._cnt > 0 & self.pseudo_temporal_shift:
            # temporal moving of object positions for pseudo video clip from a single image
            shift_temporal_range = np.minimum(max_offset, np.multiply(0.1, output_size).astype(int))
            shift_temporal_offset = ((np.random.rand(2) * 2 - 1) * shift_temporal_range).astype(int)
            offset = self.offset_save + shift_temporal_offset
            offset = np.minimum(np.maximum(offset, 0), max_offset)
...
def build_augmentation(cfg, is_train):
    ...
    if is_train:
        ...
        square_enabled = cfg.INPUT.LSJ_AUG.SQUARE_ENABLED
        if use_lsj:
            ...
            if square_enabled:
                ...
                aug_list.append(
                    ResizeShortestEdgeClip(min_size, max_size, sample_style, clip_frame_cnt=clip_frame_cnt)
                )
        ...
        sample_style = "choice"
        if cfg.INPUT.LSJ_AUG.SQUARE_ENABLED:
            aug_list.append(T.ResizeShortestEdge(min_size, min(max_size, cfg.INPUT.LSJ_AUG.IMAGE_SIZE), sample_style))
        else:
    ...
    if is_train:
        ...
        if use_lsj:
            ...
            if cfg.INPUT.PSEUDO.CROP.ENABLED:
                crop_aug = RandomApplyClip(
                    T.AugmentationList([
                        ResizeShortestEdgeClip([600, 700, 800], 1333, sample_style, clip_frame_cnt=clip_frame_cnt),
                        RandomCropClip(cfg.INPUT.PSEUDO.CROP.TYPE, cfg.INPUT.PSEUDO.CROP.SIZE, clip_frame_cnt=clip_frame_cnt)
                    ]),
                    clip_frame_cnt=clip_frame_cnt
                )
