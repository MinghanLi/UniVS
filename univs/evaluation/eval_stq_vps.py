...
def main():
    ...
    stq_metric = STQuality(
        n_classes, thing_list_, ignore_label,bit_shit, 2**24
    )
    ...
    for seq_id, video_images in enumerate(pbar):
        ...
        for imgname_j in gt_image_jsons:
            ...
            pred_pan = Image.open(os.path.join(submit_dir, 'pan_pred', video_id,imgname))
            pred_pans.append(np.array(pred_pan))
            gt_pan = Image.open(os.path.join(truth_dir, video_id,imgname))
