...
def pre_tokenize(class_names):
    ...
    # len(class names) x (len(sts) * 81) x 77), where 81 and 77 are the number of templates and the length of expression respectively
    input_ids_all_classes = torch.stack(input_ids_all, 0)
    return input_ids_all_classes


def pre_tokenize_expression(expressions):
    """
    pre-tokenize class names
    ...
    for expression in expressions:
        ...
        for pt in prompt_templates:
            ...
            this_input_ids = convert_example_to_features_bpe(expression_pt, tokenizer, sot_token, eot_token)
            input_ids.append(torch.tensor(this_input_ids, dtype=torch.long))

        input_ids_all.append(torch.stack(input_ids, 0))

    # len(expressions) x 81 x 77, where 81 and 77 are the number of templates and the length of expression respectively
