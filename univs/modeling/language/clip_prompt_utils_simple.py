...
# Code from https://github.com/openai/CLIP/blob/main/clip/simple_tokenizer.py
@lru_cache()
def default_bpe():
...
def get_prompt_templates(text_type=None):
    # prompt_templates = [
    ...
    if text_type == 'caption':
    ...
    elif text_type == 'semantic_seg':
        prompt_templates = [
            '{}',
            'a semantic segmentation of {}',
            'all {} in the whole image.',
        ]

    elif text_type == 'class_name':
        # detection task
        prompt_templates = [
            '{}',
            'a category name: {}',
        ...
        # flickr30K, a caption refers to multiple objects
        prompt_templates = [
            '{}',
        ]
...
def prompt_engineering(classnames, template=""):
    return template.replace('{}', classnames.replace('/', '').replace(',', '').replace('+', ' '))
...
def convert_example_to_features_bpe(text, tokenizer, sot_token, eot_token, context_length=77):
    ...
    return pad_input_ids, input_ids.shape[0]
...
def get_cls_names(filter_novel=False, coco=None, from_file=False):
    ...
    """
    from ..data.datasets.lvis_v1_categories import LVIS_CATEGORIES as LVIS_V1_CATEGORIES
    from ..data.datasets.coco_zeroshot_categories import (
        COCO_UNSEEN_CLS,
        COCO_SEEN_CLS,
        COCO_OVD_ALL_CLS,
        COCO_80_ALL_CLS
...
def pre_tokenize(class_names, text_type='class_name'):
    """
    ...
    # prompt engineering
    prompt_templates = get_prompt_templates(text_type)
    input_ids_all, input_ids_all_len = [], []
    for text in class_names:
        ...
        if isinstance(text, str):
            ...
            texts += text
        t1s = [prompt_engineering(text, template=pt) for pt in prompt_templates for text in texts]

        input_ids, input_ids_len = [], []
        for t1 in t1s:
            this_input_ids, this_input_ids_len = convert_example_to_features_bpe(t1, tokenizer, sot_token, eot_token)
            input_ids.append(torch.tensor(this_input_ids, dtype=torch.long))
            input_ids_len.append(this_input_ids_len)

        input_ids_all.append(torch.stack(input_ids, 0))
        input_ids_all_len.append(torch.tensor(input_ids_len))

    # len(class names) x (len(sts) * 81) x 77)
    input_ids_all_classes = torch.stack(input_ids_all, 0)
    input_ids_all_len_classes = torch.stack(input_ids_all_len)
    return input_ids_all_classes, input_ids_all_len_classes


def pre_tokenize_expression(expressions, text_type='expression'):
    """
    pre-tokenize class names
    :param expressions: a sentence of expression
    :param tokenizer: Tokenizer, SimpleTokenizer()
    :return: Tensor, containing all prompts for all classes, [#cls, #prompts, context_length]
    """
    # tokenizer
    ...
    # prompt engineering
    prompt_templates = get_prompt_templates(text_type=text_type)
    input_ids_all, input_ids_all_len = [], []
    for expression in expressions:
        assert isinstance(expression, str)
        input_ids, input_ids_len = [], []
        for pt in prompt_templates:
            expression_pt = pt.replace('{}', expression)
            this_input_ids, this_input_ids_len = convert_example_to_features_bpe(expression_pt, tokenizer, sot_token, eot_token)
            input_ids.append(torch.tensor(this_input_ids, dtype=torch.long))
            input_ids_len.append(this_input_ids_len)
    ...
    # len(expressions) x num_templates x 77, where 77 is the length of expression
    input_ids_all = torch.stack(input_ids_all, 0)
    input_ids_all_len = torch.stack(input_ids_all_len)
    return input_ids_all, input_ids_all_len
