...
from .comm import vis_clip_instances_to_coco_json_video 


class InferenceVideoVIS(nn.Module):
    """
    ...
    def eval(self, model, batched_inputs):
        ...
        if dataset_name.startswith("ytvis") or dataset_name.startswith("ovis"):
            if self.tracker_type == 'minvis':
                return self.inference_video_vis_minvis(model, batched_inputs, images_norm, targets)
            elif self.tracker_type == 'mdqe':
                return self.inference_video_vis_mdqe(model, batched_inputs, images_norm, targets=targets)
            else:
    ...
    def inference_video_vis_minvis(self, model, batched_inputs, images, targets):
        ...
        for i in range(len(images_tensor)):
            ...
            pred_embds = out['pred_embds'][0, :self.num_queries].float()   # QxTxC->QxC
            pred_embds = pred_embds.mean(1)
            if i == 0:
        ...
        if self.zero_shot_inference:
            mask_scores = (mask_scores * 0.5).softmax(-1)  # cos_sim without L2 norm
        else:
    ...
    def inference_video_vis_minvis_save_video(self, model, images, outputs, interim_size, image_size, out_size):
        ...
        mask_quality_scores = calculate_mask_quality_scores(mask_pred[:, ::itv_t])
    ...
    def match_from_embds(self, tgt_embds, cur_embds, return_scores=False):
        cur_embds = cur_embds / cur_embds.norm(dim=-1)[..., None]
        tgt_embds = tgt_embds / tgt_embds.norm(dim=-1)[..., None]
        cos_sim = torch.mm(tgt_embds, cur_embds.transpose(0, 1))
    ...
    def inference_video_vis_mdqe(self, model, batched_inputs, images, targets):
        ...
        for i in range(video_len):
            ...
            outputs_per_window = self.inference_video_vis_mdqe_per_clip(
                tracker, outputs, frame_idx_clip, dataset_name, is_first_clip, is_last_clip
            )
    ...
    def inference_video_vis_mdqe_per_clip(
        self, tracker, outputs, frame_idx_clip, dataset_name, is_first_clip=False, is_last_clip=False
    ):
        # remove prompt queries here 
        ...
        pred_embds = outputs['pred_embds'][0, :self.num_queries].float().mean(1)  # q t c -> q c
        ...
        if not is_first_clip:
            mem_embds = tracker.get_query_embds()
            mem_indices, mem_scores = self.match_from_embds(mem_embds, pred_embds, return_scores=True)
            ...
            if len(mem_indices) > 0:
                sim_mem = torch.mm(
                    ...
                    F.normalize(pred_embds[mem_indices]).t(),
                )
        ...
        if len(valid_idx):
            ...
            if enable_mask_iou:
                ...
                valid_idx = valid_idx[max_sim < cos_sim_thresh_bottom]
    ...
    def inference_video_vis_mdqe_save_window(self, cur_frame_idx, batched_inputs, images, outputs_window, is_last_clip):
        ...
        mask_quality_scores = calculate_mask_quality_scores(pred_masks)
        pred_scores = pred_scores * mask_quality_scores.view(-1, 1)
        ...
        for obj_id, s, m in zip(pred_obj_ids, pred_scores, pred_masks_list):
            segms = [
