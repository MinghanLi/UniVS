...
class ModifiedResNet(Backbone):
    ...
    def __init__(self, layers, output_dim, heads, input_resolution=224, width=64,
                 out_features=None, freeze_at=0, depth=None, pool_vec=True, create_att_pool=False,
                 norm_type='FronzenBN'):
        super().__init__()
        ...
        if depth in [50, 101]:  # resnet50 or resnet 101
            ...
            self._out_feature_channels = {'stem': 64, 'res2': 256, 'res3': 512, 'res4': 1024,
                                          'res5': 2048} if 'res5' in self._out_features \
                else {'stem': 64, 'res2': 256, 'res3': 512, 'res4': 1024}
            self._out_feature_strides = {'stem': 4, 'res2': 4, 'res3': 8, 'res4': 16,
                                         'res5': 32} if 'res5' in self._out_features \
                else {'stem': 4, 'res2': 4, 'res3': 8, 'res4': 16}  # anti-aliasing strided conv???
        elif depth in [200]:  # resnet50x4
            # FPN: ["res2", "res3", "res4", "res5"]; C4: ["res4"]
            self._out_feature_channels = {'stem': 80, 'res2': 320, 'res3': 640, 'res4': 1280,
                                          'res5': 2560} if 'res5' in self._out_features \
    ...
    def forward(self, x):
        ...
        x = self.layer4(
            x) if "res5" in self._out_features else x  # det2 resnet50: [2048, 25, 38]; CLIP resnet50: [2048, 7, 7]
        outputs['res5'] = x if "res5" in self._out_features else None

        if self.pool_vec:
            # pool a vector representation for an image, for global image classification, HW, N, C -> N, C
...
@BACKBONE_REGISTRY.register()
def build_clip_resnet_backbone(cfg, input_shape=None):
    """
    ...
    out_features = cfg.MODEL.CLIP.RESNETS_OUT_FEATURES
