# original code: https://github.com/wjn922/ReferFormer/blob/main/tools/data/convert_davis_to_ytvos.py
import os
import json
...
def read_split_set(data_root='datasets/ref-davis'):
...
def mv_images_to_folder(data_root='datasets/ref-davis', output_root='datasets/ref-davis'):
...
def create_meta_expressions(data_root='datasets/ref-davis', output_root='datasets/ref-davis'):
    """
...
def create_meta_annotaions(data_root='datasets/ref-davis', output_root='datasets/ref-davis'):
    """
...
def convert_to_ytvis_format(data_root='datasets/ref-davis', output_root='datasets/ref-davis'):
    
    meta_exp_val_path = os.path.join(output_root, "meta_expressions/valid", "meta_expressions.json")
    meta_val_path = os.path.join(output_root, "valid", "meta.json")

    with open(meta_exp_val_path) as f:
        meta_exp_data = json.load(f)
    
    with open(meta_val_path) as f:
        meta_data = json.load(f)

    print(meta_exp_data, meta_data)
    exit()
...
if __name__ == '__main__':
    ...
    convert_to_ytvis_format(data_root, output_root)