...
def convert_to_coco(data_root='datasets/refcoco', output_root='datasets/refcoco', dataset='refcoco', dataset_split='unc'):
    ...
    for split in splits:
        ...
        for i in tqdm(ref_ids): 
            ...
            img_id = int(img["file_name"].split("_")[-1][:-4])
            if img_id not in img_ids:
                img_ids[img["id"]] = img_id
                # append image info
                image_info = {
        ...
        print("Total images {} and expressions: {} in split {}".format(
            len(coco_ann["images"]), len(coco_ann["annotations"]), split)
        )
