...
if __name__ == "__main__":
    ...
    for split in splits:
        ...
        output_json = os.path.join(data_root, "%s.json"%split)
        json.dump(new_data, open(output_json, 'w'))
        
# the differnence between train.json and val.json
# in val.json, only one frame annotation for each object is given, other frames are set to None