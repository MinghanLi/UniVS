...
if __name__ == "__main__":
    ...
    splits = ["valid", "valid_sub"]
    for split in splits:
        assert split.startswith("valid")
        new_data = {"videos": [], "annotations": [], "categories": [{"supercategory": "object","id": 1,"name": "object"}]}
        data_dir = os.path.join(data_root, split.split('_')[0])
        ...
        for _, vid in enumerate(video_names):
            ...
            if split == "valid_sub" and vid_idx >= 50:
                # only use 50 videos for fast evalution, named as dev set or val_sub set
                break
            ...
            for obj_id in obj_ids:
                ...
                if (mask_cur > 0).any():
                    if obj_id not in vid_obj_dict:
                        vid_obj_dict[obj_id] = {
                            "video_id": vid_idx, "id": obj_id, "iscrowd": 0, "category_id": 1,
